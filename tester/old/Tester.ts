...
import * as tdHelpers from '/home/eko/Code/node-wot/packages/node-wot-td-tools/src/td-helpers'
...
export class Tester {
    ...
    */
    public testProperty(testCycle: number, propertyName: string, testScenario: number, interactionIndex:number,logMode: boolean): Promise<any> {
        ...
        return new Promise(function (resolve, reject) {
            ...
            try {
                ...
                if (logMode) logger.error("The property " + propertyName + " doesn't exist in the TD");
            ...
            let curPropertyData: any = self.tut.getProperty(propertyName).then((res: any) => {

                let data: JSON = res;
                if (logMode) logger.info("Gotten propery data is ", data);
                ...
                if (errorsProp.length > 0) { //meaning that there is a validation error
                    if (logMode) logger.error("Received response is not valid for  " + propertyName, errorsProp);
                    ...
                    if (logMode) logger.info("Property test of " + propertyName + " is succesful")
                ...
                if (isWritable) { //if we can write into the property, it means that we can test whether we can write and get back the same type
                    ...
                    if (logMode) logger.info("Testing the write functionality for ", propertyName);
                    let toSend: JSON;
                    let answer: JSON;
                    //generating the message to send 
                    try {
                        ...
                        if (logMode) logger.error("Cannot create message for " + propertyName + ", look at the previous message to identify the problem");
                    ...
                    if (errors.length > 0) { //meaning that there is a validation error
                        if (logMode) logger.error("Created request is not valid for " + propertyName + "\nMessage is " + toSend + "\nError is " + errors);
                    ...
                    if (logMode) logger.info("Setting property " + propertyName + " with toSend = ", toSend)
                    self.tut.setProperty(propertyName, toSend).then(() => {
                        //now reading and hoping to get the same value
                        let curPropertyData2: any = self.tut.getProperty(propertyName).then((res2: any) => {
                            let data2: JSON = res2;
                            if (logMode) logger.info("For the second one, gotten propery data is ", data2);
                            ...
                            if (errorsProp2.length > 0) { //meaning that there is a validation error
                                if (logMode) logger.error("Received second response is not valid for  " + propertyName, errorsProp2);
                                ...
                                if (JSON.stringify(data2) == JSON.stringify(toSend)) {
                                    ...
                                    if (logMode) logger.info("Property test of " + propertyName + " is succesful");
                                    if (logMode) logger.info("The value gotten after writing is the same for the property " + propertyName);
                                    ...
                                    if (logMode) logger.error("Property test of " + propertyName + " is succesful BUT");
                                    if (logMode) logger.error("The second get didn't match the write")
                            ...
                            if (logMode) logger.error("Problem fetching property " + propertyName + "in the second get");
                        ...
                        if (logMode) logger.error("Couldn't set the property " + propertyName);
                ...
                if (logMode) logger.error("Problem fetching property " + propertyName);
    ...
    private testInteraction(testCycle: number, testScenario: number, interactionIndex:number ,interaction: TD.Interaction, logMode: boolean): Promise<any> {
        var self = this;
        return new Promise(function (resolve, reject) {
            if (interaction.semanticTypes.indexOf("Property") > -1) { //testing a property
                let propName: string = interaction.name;
                if (logMode) logger.info(" Testing Property ", propName); // the i alue is put just to be able to track the order
                self.testProperty(testCycle, propName, testScenario,interactionIndex, logMode).then((curBool) => {
                    resolve(curBool);
                }).catch((curBool) => {
                    if (logMode) logger.error("Error in testing property ", propName, ", check previous messages")
                    reject(curBool);
                });

            } else if (interaction.semanticTypes.indexOf("Action") > -1) { //testing an action
                let actName: string = interaction.name;
                if (logMode) logger.info(" Testing Action ", actName);// the i alue is put just to be able to track the order
                self.testAction(testCycle, actName, testScenario,interactionIndex, logMode).then((curBool) => {
                    resolve(curBool);
                }).catch((curBool) => {
                    if (logMode) logger.error("Error in testing action ", actName, ", check previous messages")
                    reject(curBool);
                });
            } else {
                if (logMode) logger.error("Asked for something other than Action or Property")
                reject(false);
            }
        });
    }
    ...
    */
    public testScenario(testCycle: number, testScenario: number, logMode: boolean): Promise<any> {
        ...
        */
        ...
        return new Promise(function (resolve, reject) {
            ...
            promise.then(() => {
                if (logMode) logger.info("Test Scenario nb", testScenario, " has finished")
                resolve();
            }).catch((error:Error) => {
                if (logMode) logger.error("Test Scenario nb", testScenario, " has finished with an error")
                reject(error);
            });
        });
    ...
    public testCycle(cycleNumber: number, logMode: boolean): Promise<any> {
        ...
        return new Promise(function (resolve, reject) {
            ...
            promise.then(() => {
                if (logMode) logger.info("Test Cycle nb", cycleNumber, " has finished")
                resolve();
            }).catch(() => {
                if (logMode) logger.error("Test Cycle nb", cycleNumber, " has finished with an error")
                reject();
            });
        });
    }
    ...
    */
    ...
    public testThing(repetition: number, logMode: boolean): Promise<TestReport> {
        ...
        return new Promise(function (resolve, reject) {
            ...
            reps.forEach(repNb => {
                promise = promise.then(() => {
                    if (logMode) logger.info("Cycle " + repNb + ", testing all scenarios")
                    self.testReport.addTestCycle();
                    return self.testCycle(repNb, logMode);
                });
            });
            promise.then(() => {
                if (logMode) logger.info("Testing the Thing has finished")
                resolve(self.testReport);
            }).catch(() => {
                if (logMode) logger.error("Testing the Thing has finished with an error")
                reject();
            });
        });
    }
